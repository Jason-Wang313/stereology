...
Let $\mathcal{M} = \{m_1, \ldots, m_n\}$ denote a population of $n$ models, each possessing a \emph{capability profile} $c(m_i) \in \mathbb{R}^D$ for some unknown $D$. A benchmark suite $\Pi = \{\pi_1, \ldots, \pi_k\}$ maps each profile to an observable score vector:
\[
  \Pi(m_i) = (\pi_1(m_i), \ldots, \pi_k(m_i)) \in \mathbb{R}^k.
\]
We observe the \emph{score matrix} $S \in \mathbb{R}^{n \times k}$ with $S_{ij} = \pi_j(m_i)$.
...
\]
Wait — more carefully: the captured variance probes $d_{\mathrm{eff}}$ directions. In the worst case (benchmarks aligned with top eigenvectors of $\Sigma_C$), the captured variance equals $\sum_{i=1}^{d_{\mathrm{eff}}} \mu_i$, which is maximized at $d_{\mathrm{eff}} \cdot \mu_1$. But as a fraction of total variance:
\[
