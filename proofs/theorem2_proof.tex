...
\label{cor:ranking}
Consider $n$ models ranked by aggregate benchmark score $\bar{s}(m_i) = \frac{1}{k}\sum_j \pi_j(m_i)$. Let $\Delta_{\min} = \min_{i \neq j} |\bar{s}(m_i) - \bar{s}(m_j)|$ be the minimum score gap. If the indistinguishability radius $\delta_0 = \pi R / m$ exceeds $\Delta_{\min}$, then there exist $(\varepsilon, \Pi)$-indistinguishable pairs that swap rankings:
\[
...
\[
  P(\text{top-1 correct}) \leq 1 - \binom{n}{2}^{-1} \sum_{i < j} \mathbf{1}\!\left[\frac{|\bar{s}(m_i) - \bar{s}(m_j)|}{\delta_0} < 1\right].
\]
\end{corollary}
...
\label{cor:reversal}
If $d_{\mathrm{eff}} < n - 1$, then there exist model populations where adding a single model $m_{n+1}$ to the ranked set reverses the relative ordering of two existing models $m_i, m_j$.
\end{corollary}

\begin{proof}
When $d_{\mathrm{eff}} < n - 1$, the $n$ model score vectors $\Pi(m_1), \ldots, \Pi(m_n) \in \mathbb{R}^k$ lie in a subspace of dimension $d_{\mathrm{eff}} < n - 1$. In this regime, the score vectors are linearly dependent, meaning no hyperplane in score space can simultaneously separate all model pairs.

The aggregate score $\bar{s}(m_i) = \mathbf{w}^\top \Pi(m_i)$ for weight vector $\mathbf{w} = (1/k, \ldots, 1/k)$ defines a linear functional on the score subspace. Adding a model $m_{n+1}$ whose score vector is not in the span of $\{\Pi(m_i)\}_{i=1}^n$ can alter the weight vector that ``best'' aggregates scores (e.g., via normalization or re-weighting), which can reverse the sign of $\bar{s}(m_i) - \bar{s}(m_j)$ for some pair.
